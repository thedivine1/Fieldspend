...
function groupByDay(
  ...
  for (const [dateStr, items] of map.entries()) {
    // Sort by createdAt descending as default order
...
interface DayGroupCardProps {
  ...
  onEdit: (r: Receipt) => void;
  onDelete: (id: string) => void;
  onReorder: (dateStr: string, newOrder: Receipt[]) => void;
}
...
function DayGroupCard({
  ...
  return (
    ...
    >
      {/* Day header */}
      <div className="daily-header sticky top-[57px] z-10 backdrop-blur-sm">
...
export default function GalleryPage() {
  ...
  // Mark loading complete after first render (IndexedDB hydration is synchronous via zustand persist)
  useEffect(() => {
    setIsLoadingReceipts(false);
  }, []);

  // Local reorder state per day (overrides store order within a session)
  ...
  const handleReorder = useCallback((dateStr: string, newOrder: Receipt[]) => {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    // Clear any local reorder entries that reference this id
    setDayOrders((prev) => {
  ...
  return (
    <div className="flex flex-col min-h-full" data-ocid="gallery.page">
      {/* Month selector */}
      <MonthSelector />
